...
//                        Toast.makeText(context.getApplicationContext(), uri.toString(), Toast.LENGTH_LONG).show();
                        ...
                        // Send success broadcast with download uri
                        broadcastDownloadFinished(downloadPath, uri, 1);
                        showDownloadFinishedNotification(downloadPath, uri, 1);
                ...
                .addOnFailureListener(new OnFailureListener() {
                    ...
                    public void onFailure(@NonNull Exception exception) {
                        ...
                        Uri fail = Uri.parse("Download Failed");

                        // Send failure broadcast
                        broadcastDownloadFinished(downloadPath, fail,-1);
                        showDownloadFinishedNotification(downloadPath, fail , -1);
     ...
     */
    private boolean broadcastDownloadFinished(String downloadPath, Uri uri, int bool) {
        boolean success = bool != -1;
        String action = success ? DOWNLOAD_COMPLETED : DOWNLOAD_ERROR;

        Intent broadcast = new Intent(action)
                .putExtra(EXTRA_DOWNLOAD_PATH, downloadPath)
                .putExtra(EXTRA_DOWNLOAD_URL, uri);
     ...
     */
    private void showDownloadFinishedNotification(String downloadPath, Uri uri,int bool) {
        ...
        Context context = getApplicationContext();
        //Toast.makeText(context.getApplicationContext(), intent.getParcelableExtra(EXTRA_DOWNLOAD_URL).toString(), Toast.LENGTH_LONG).show();
